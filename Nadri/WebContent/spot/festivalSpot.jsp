	...
	function initMap() {
		...
		for (var i = 0; i < locations.length; i++) {
			...
			contents[i] = '<div class="box box-primary">'
					...
					+ ' </ul>'
					+ '<span> '
					+ ' <a href="/spot/getFestival?spotNo='+ locations[i].contentid+ '" class="btn btn-secondary btn-block"><b>상세보기</b></a>'
					+ '<span>'
					+ '<span> '
					+ ' <a href="#" id ="marking" class="btn btn-secondary btn-block" onclick="marking()"><b>장소바구니추가</b></a>'
					+ '</span>' + '</div>';
	...
	<body>
		...
		<span class="dropdown">
			<button class="btn btn-block btn-info btn-xs">
				<a href="/spot/getSpotList?spotCode=0">공원</a>
			</button>
		</span>

		<span class="dropdown">
			<button class="btn btn-block btn-success btn-xs">
				<a href="/spot/getFestivalList">축제/전시</a>
			</button>
</div>
</span>
...
<span class="dropdown">
	...
	<div class="dropdown-content">
		<a href="/spot/getSpotList?spotCode=10">삼대천왕</a> <a
			href="/spot/getSpotList?spotCode=11">수요미식회</a>
	</div>
</span>

<span class="dropdown">
	<button class="btn btn-block btn-danger btn-xs">
		<a href="/spot/getSpotList?spotCode=4">한강</a>
	</button>
</span>
...
<span class="dropdown">
	...
	<div class="dropdown-content">
		<a href="/spot/getSpotList?spotCode=30">수유실</a> <a
			href="/spot/getSpotList?spotCode=31">자동차</a> <a
			href="/spot/getSpotList?spotCode=32">자전거</a>
	</div>
</span>

<span class="dropdown">
	<button class="btn btn-block btn-normal btn-xs">
		<a href="/spot/getSearchSpot">직접검색</a>
	</button>
</span>
	...
	<c:forEach var="festival" items="${a.response.body.items.item}">
		...
		<tr class="ct_list_pop">
			...
			<c:if test="${ !empty festival.firstimage}">
				<!--  값이 있을때 -->
				<td align="center"><img src="${festival.firstimage}"
					width="200" height="100" /></td>
			</c:if>
			<c:if test="${ empty festival.firstimage}">
				<td align="center"><img src="/images/spot/festivaldefault.jpg"
					width="200" height="100" /></td>
			</c:if>
			<td align="center">${festival.addr1}</td>
			<td align="center">${festival.tel}</td>
			<td align="center">${festival.sigungucode}<span
				class="label label-info">축제/전시</span></td>
